...
<body>
	...
	<%
		...
		PreparedStatement check = con.prepareStatement("SELECT item_id, username FROM auto_bid WHERE item_id = ? AND username = ?");
		check.setString(1, item_id);
		...
		if(min_bid == null || max_bid == null || increments == null){
			...
			if (rs.next() == false){
				...
				%>
					...
					<tr><td>Current Bid:&emsp;&emsp;</td><td> <%=current_bid%> $</td></tr>
					<tr><td>Closing Date and Time:&emsp;&emsp;</td><td> <%= rs.getString("closing_date_time") %> </td></tr>
					</table>
					<h2 style="color: lightcyan;">Automatic Bid Set-up</h2>
					<table>
						
						<tr><td>Minimum Bid:&emsp;&emsp;</td><td><input type="number" name="min_bid" min="<%=current_bid%>" required>$</td></tr>
						<tr><td>Maximum Bid:&emsp;&emsp;</td><td><input type="number" name="max_bid" min="<%=Integer.parseInt(current_bid)+1%>"required>$</td></tr>
						<tr><td>Increments:&emsp;&emsp;</td><td><input type="number" name="increments" min="1" required>$</td></tr>
			...
			PreparedStatement ps2 = con.prepareStatement("INSERT INTO auto_bid (item_id, username, increments, min_bid, max_bid) VALUES (?, ?, ?, ?, ?)");
			ps2.setString(1, item_id);
			ps2.setString(2, username);
			ps2.setString(3, increments);
			ps2.setString(4, min_bid);
			ps2.setString(5, max_bid);
			ps2.executeUpdate();
